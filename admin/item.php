...
function addItemRestaurants($pdo, $item_id) {
    if (isset($_POST['restaurants_list'])) {
        $list = explode(',', $_POST['restaurants_list']);
        $in  = str_repeat('?,', count($list) - 1) . '?';
        $stmt = $pdo->prepare('DELETE FROM items_restaurants WHERE item_id = ? AND restaurant_id NOT IN (' . $in . ')');
        $stmt->execute(array_merge([ $item_id ], $list));
...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        addItemRestaurants($pdo, $_GET['id']);
    ...
    if (isset($_POST['delete'])) {
        ...
        $stmt = $pdo->prepare('DELETE i, ii, io, ic FROM items i LEFT JOIN items_images ii ON ii.item_id = i.id LEFT JOIN items_options io ON io.item_id = i.id LEFT JOIN items_restaurants ir ON ic.item_id = i.id LEFT JOIN items_categories ic ON ic.item_id = i.id WHERE i.id = ?');
        $stmt->execute([ $_GET['id'] ]);
        header('Location: index.php?page=items');
        exit;
    }
